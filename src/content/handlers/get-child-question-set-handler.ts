        ...
        try {
            for await (const child of questionSet.children) {
                if (child.objectType === 'QuestionSet') {
                    ...
                    const question = await this.fileService.readAsText(`${path}content/${questionSet.identifier}/${child.identifier}`, 'index.json').catch(e => { new Error(e) });
            ...
            throw new Error(`Failed to read question: ${error}`);
        }
